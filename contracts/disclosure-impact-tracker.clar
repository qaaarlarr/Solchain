...
;; Finalize impact assessment after voting period
(define-public (finalize-impact-assessment (disclosure-id uint))
  (let
    (
      (impact-data (unwrap! (map-get? disclosure-impacts { disclosure-id: disclosure-id }) err-not-found))
      (disclosure-data (unwrap! (contract-call? .Truthdrop get-disclosure disclosure-id) err-not-found))
    ...
    (asserts! (>= stacks-block-height (get assessment-deadline impact-data)) err-voting-closed)
    (asserts! (not (get final-assessment impact-data)) err-already-voted)
    (asserts! (>= (get total-votes impact-data) (var-get impact-assessment-threshold)) err-insufficient-reputation)
    
    ;; Mark as finalized and update user history
    (map-set disclosure-impacts
      { disclosure-id: disclosure-id }
      (merge impact-data { final-assessment: true })
    )
    
    (try! (update-user-impact-history (get whistleblower disclosure-data) (get impact-score impact-data) (get impact-tier impact-data)))
    (try! (update-platform-impact-stats (get impact-score impact-data) (get impact-tier impact-data)))
    (ok true)
  )
)
...
;; Update bounty creator's impact bonus based on successful high-impact bounties
(define-public (update-creator-impact-bonus (creator principal) (bounty-had-high-impact bool))
  (let
    (
      (creator-bonus (default-to
        { successful-high-impact-bounties: u0, total-bounties-created: u0, 
          impact-success-rate: u0, current-multiplier: u10000, last-updated: u0 }
        (map-get? creator-impact-bonuses { creator: creator })))
    )
    (let
      (
        (new-total-bounties (+ (get total-bounties-created creator-bonus) u1))
        (new-high-impact-count (if bounty-had-high-impact 
                                 (+ (get successful-high-impact-bounties creator-bonus) u1)
                                 (get successful-high-impact-bounties creator-bonus)))
        (new-success-rate (if (> new-total-bounties u0) 
                            (/ (* new-high-impact-count u100) new-total-bounties) 
                            u0))
        (new-multiplier (+ u10000 (* new-success-rate u50))) ;; Up to 5x multiplier for 100% success rate
      )
      (map-set creator-impact-bonuses
        { creator: creator }
        {
          successful-high-impact-bounties: new-high-impact-count,
          total-bounties-created: new-total-bounties,
          impact-success-rate: new-success-rate,
...
(define-private (update-user-impact-history (user principal) (impact-score uint) (impact-tier uint))
  (let
    (
      (history (default-to 
        { total-disclosures: u0, high-impact-count: u0, average-impact-score: u0, 
          impact-reputation: u0, last-high-impact-block: u0 }
        (map-get? user-impact-history { user: user })))
      (is-high-impact (>= impact-tier u4))
    )
    (let
      (
        (new-total (+ (get total-disclosures history) u1))
        (new-high-impact (if is-high-impact 
                           (+ (get high-impact-count history) u1) 
                           (get high-impact-count history)))
        (new-avg-score (/ (+ (* (get average-impact-score history) (get total-disclosures history)) impact-score) new-total))
        (new-reputation (+ (get impact-reputation history) (* impact-score u1)))
      )
      (map-set user-impact-history
        { user: user }
        {
          total-disclosures: new-total,
          high-impact-count: new-high-impact,
          average-impact-score: new-avg-score,
          impact-reputation: new-reputation,
          last-high-impact-block: (if is-high-impact stacks-block-height (get last-high-impact-block history))
        }
      )
      (ok true)
    )
...
(define-private (update-platform-impact-stats (impact-score uint) (impact-tier uint))
  (let
    (
      (current-period "current")
      (stats (default-to 
        { total-assessed-disclosures: u0, average-impact-score: u0, 
          high-impact-percentage: u0, policy-changes-triggered: u0, regulatory-actions-count: u0 }
        (map-get? platform-impact-stats { period-key: current-period })))
    )
    (let
      (
        (new-total (+ (get total-assessed-disclosures stats) u1))
        (new-avg (/ (+ (* (get average-impact-score stats) (get total-assessed-disclosures stats)) impact-score) new-total))
        (is-high-impact (>= impact-tier u4))
        (high-impact-count (if is-high-impact u1 u0))
        (new-high-percentage (/ (* high-impact-count u100) new-total))
      )
      (map-set platform-impact-stats
        { period-key: current-period }
        (merge stats {
          total-assessed-disclosures: new-total,
          average-impact-score: new-avg,
          high-impact-percentage: new-high-percentage
        })
      )
      (ok true)
    )
